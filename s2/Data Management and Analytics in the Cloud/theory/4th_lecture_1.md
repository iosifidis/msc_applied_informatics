...
*   **Ευελιξία:** Είναι χρήσιμες για **scalable data setups** (κατανεμημένες και κλιμακούμενες αρχιτεκτονικές), μπορούν να διαχειριστούν **αλλαγές σχήματος** (schema changes) στην τιμή (value), και να αποθηκεύσουν δεδομένα για πιο **ευέλικτη ανάκτηση** (flexble retrieval) *βάσει του κλειδιού*.
...
*   **Υποστήριξη Τύπου Δεδομένων (Value) (Slide 6):**
    *   Η *τιμή (value)* σε ένα ζεύγος κλειδί-τιμή μπορεί να είναι οτιδήποτε - από μια απλή στοιχειοσειρά (string) ή αριθμό, μέχρι πιο σύνθετες δομές.
...
*   **Υποστήριξη ACID (Slide 10):**
    *   Πολλά από τα κέρδη στην απόδοση και κλιμακούμενοτητα των NoSQL, και των KV ειδικότερα, προέρχονται από την **απόρριψη της υποστήριξης για ACID συναλλαγές** (που είναι ακριβές σε κατανεμημένα περιβάλλοντα).
...
*   **Κλιμακούμενοτητα (Scalability) (Slide 11):**
...
*   **Berkeley DB (Slide 21):**
    ...
    *   Ήταν μια **ενσωματωμένη (embedded)** KV βάση, σχεδιασμένη να λειτουργεί *μέσα* στην εφαρμογή, όχι ως ξεχωριστός server με δικτυακή πρόσβαση.
...
*   **Πώς Επιλύουμε Συγκρούσεις (με Vector Clocks ή αλλιώς) (Slide 40):**
    ...
    *   **Keep both copies:** Κρατάμε *και τα δύο* συγκρουόμενα αντίγραφα και αφήνουμε την **εφαρμογή ή τον χρήστη** να επιλύσει τη σύγκρουση (π.χ. χειροκίνητα merge ή με συγκεκριμένη λογική στην εφαρμογή).
    ...
    *   **Conflict-Free Replicated Data Type (CRDT - Τύπος Δεδομένων χωρίς Συγκρούσεις) (Slides 40-41):** Ειδικοί τύποι δεδομένων που είναι **μαθηματικά σχεδιασμένοι** έτσι ώστε οι ταυτόχρονες ενημερώσεις να μπορούν να εφαρμοστούν με *οποιαδήποτε σειρά* σε οποιοδήποτε αντίγραφο, και τελικά όλα τα αντίγραφα να συγκλίνουν (converge) στην **ίδια τελική κατάσταση** χωρίς να απαιτείται ειδική λογική επίλυσης συγκρούσεων (πέρα από την εφαρμογή όλων των operation). Ένα παράδειγμα είναι το `g-counter` (grow-only counter) που μόνο αυξάνει. Χρησιμοποιούνται σε συστήματα όπως το Riak.
...
*   **Local Indexes (Τοπικά Ευρετήρια) (Slide 44):**
    ...
    *   Ερωτήματα που βασίζονται στο ευρετήριο **αποστέλλονται σε όλους τους κόμβους (broadcast)**. Κάθε κόμβος χρησιμοποιεί το τοπικό του ευρετήριο για να βρει τυχόν ταιριασμοί στα *δικά του* δεδομένα.
    ...
    *   Αυτή η προσέγγιση είναι καλή για την τοπική αναζήτηση, αλλά η ανάγκη για broadcast του ερωτήματος σε *όλους* τους κόμβους μπορεί να είναι αναποτελεσματική σε πολύ μεγάλα clusters, εκτός αν η συνάρτηση διαμερισμού του ευρετηρίου "ταιριάζει" με την συνάρτηση διαμερισμού του βασικού πίνακα.
