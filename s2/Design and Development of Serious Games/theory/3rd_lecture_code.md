...
```
**Επεξήγηση:** Δημιουργεί σώμα με προεπιλεγμένες παραμέτρους (μέγεθος 20, μάζα 300, χρώμα κίτρινο).
...
```
**Επεξήγηση:** Δημιουργεί σώμα με συγκεκριμένο μέγεθος, μάζα, διάνυσμα κίνησης και χρώμα (εμφανίζεται ως γεμισμένος κύκλος).

---

## **2. Βασική Συμπεριφορά**
```java
public void act() {
    applyForces();
    move();
...
```
**Επεξήγηση:** Κεντρική μέθοδος που καλείται σε κάθε κύκλο προσομοίωσης - εφαρμόζει βαρυτικές δυνάμεις, κινεί το σώμα και ελέγχει αν χτύπησε τα όρια.
...
```
**Επεξήγηση:** Εφαρμόζει βαρυτικές δυνάμεις από όλα τα άλλα σώματα και περιορίζει την μέγιστη ταχύτητα.
...
```
**Επεξήγηση:** Υπολογίζει και εφαρμόζει τη βαρυτική δύναμη μεταξύ δύο σωμάτων σύμφωνα με τον νόμο της παγκόσμιας έλξης.

---

## **4. Μέθοδοι Αλληλεπίδρασης με Όρια**
```java
...
```
**Επεξήγηση:** Ανιχνεύει σύγκρουση με τα όρια του κόσμου και προκαλεί "αναπήδηση" με απώλεια ενέργειας (επιβράδυνση 10%).
...
```
**Επεξήγηση:** Επιστρέφει τη μάζα του σώματος.

---

### **Σημειώσεις**
- Η κλάση `Body` επεκτείνει την `SmoothMover` για ομαλή κίνηση με ακριβείς συντεταγμένες.
- Χρησιμοποιεί τον νόμο της παγκόσμιας έλξης του Νεύτωνα (`F = G*(m1*m2)/r²`).
- Η σταθερά `GRAVITY` (7.8) καθορίζει την ένταση της βαρυτικής έλξης.
- Τα σώματα εμφανίζονται ως γεμισμένοι κύκλοι με καθορισμένο μέγεθος και χρώμα.
- Υπάρχει μηχανισμός περιορισμού μέγιστης ταχύτητας και ελαστικών συγκρούσεων με τα όρια.
