...
- **Resources**: Ποσότητα πόρου **καταναλώνει η κάθε εργασία**.   
...
- **Reified disjunction:**   
  Όταν ο ένας περιορισμός αποδειχθεί ψευδής, τότε επέβαλε (enforce) τον άλλο.   
  ```
  S1 + d1 ≤ S2 \/ S2 + d2 ≤ S1
  eet(S1) ≤ lst(S2) \/ eet(S2) ≤ lst(S1)
  ```

- **Constructive disjunction:**   
  Για κάθε όρο της διάζευξης (dis1 \/ dis2 \/...disn), επέλεξε το disi και κάνε τα πεδία συνεπή με τις αντίστοιχες διαγραφές τιμών (n βήματα).   

- **Edge Finding:**  
  Αναγνωρίζει εργασίες που πρέπει να εκτελεστούν πριν ή μετά από άλλες, με βάση τη χωρητικότητα των πόρων.  
  Αν ο συνολικός χρόνος ενός συνόλου εργασιών ξεπερνά το μέγιστο διαθέσιμο χρόνο, τότε ανιχνεύεται ασυνέπεια.   
  ```
  letΩ - estΩ < dΩ (Ασυνέπεια)
  ```
