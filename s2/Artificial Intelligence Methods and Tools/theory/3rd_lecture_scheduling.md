# Χρονοπρογραμματισμός στη Μοντελοποίηση Προβλημάτων
...
2.  **Είδη Εργασιών (ως προς τη Διακοπή):**
    ...
    *   **Προεκτοπιστικές (Preemptive):** Η εκτέλεση μιας εργασίας μπορεί να διακοπεί και να συνεχιστεί αργότερα. Η συνολική διάρκεια `dᵢ` είναι το άθροισμα των διαστημάτων εκτέλεσης (`dᵢ = Σ(dₖᵢ)`). Μπορεί να υπάρχουν περιορισμοί στα διαστήματα διακοπής.
...
4.  **Τύποι Πόρων:**
    *   **Επαναχρησιμοποιήσιμοι (Reusable):** Δεσμεύονται από μια εργασία για τη διάρκειά της και απελευθερώνονται μετά την ολοκλήρωσή της (π.χ., μηχανές, εργατικό δυναμικό). Έχουν **συνολική χωρητικότητα (capacity ``)** η οποία μπορεί να είναι 1 (unary resource) ή >1. Η συνολική χρήση του πόρου σε κάθε χρονική στιγμή `t` δεν πρέπει να υπερβαίνει το `Qk`.
