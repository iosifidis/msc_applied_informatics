...
*   Ένα CSP ορίζεται από μια τριάδα (V, D, C):
    ...
    *   **C:** Ένα σύνολο m **περιορισμών** {C₁, C₂, ..., C<0xE2><0x82><0x98>}. Κάθε περιορισμός C<0xE2><0x82><0x9A>:
...
4.  **Συνέπεια Τόξου (Arc Consistency - AC):**
    ...
    *   Ένα **τόξο (arc)** (Vᵢ, V<0xE2><0x82><0x9A>) είναι συνεπές αν για *κάθε* τιμή x στο πεδίο Dᵢ της Vᵢ, υπάρχει *τουλάχιστον μία* τιμή y στο πεδίο D<0xE2><0x82><0x9A> της V<0xE2><0x82><0x9A> τέτοια ώστε η ανάθεση (Vᵢ=x, V<0xE2><0x82><0x9A>=y) να ικανοποιεί τον δυαδικό περιορισμό μεταξύ τους.
    *   Αν μια τιμή x της Vᵢ δεν έχει "υποστήριξη" (support) στην V<0xE2><0x82><0x9A>, τότε η x μπορεί να αφαιρεθεί με ασφάλεια από το Dᵢ.
    *   Ένα CSP είναι **τόξο-συνεπές (arc-consistent)** αν *όλα* τα τόξα του είναι συνεπή.
    *   **Αλγόριθμος AC-3:** Ο κλασικός αλγόριθμος για την επίτευξη AC. Διατηρεί μια λίστα με τα τόξα προς έλεγχο. Όταν αφαιρεί μια τιμή από ένα πεδίο Dᵢ λόγω του τόξου (Vᵢ, V<0xE2><0x82><0x9A>), πρέπει να ξαναελέγξει όλα τα άλλα τόξα (V<0xE2><0x82><0x96>, Vᵢ) που καταλήγουν στην Vᵢ. Συνεχίζει μέχρι να μην αφαιρούνται άλλες τιμές.
...
3.  **Στρατηγικές:**
    *   **Forward Checking (FC):** Μετά την ανάθεση μιας τιμής Vᵢ=x, έλεγξε *μόνο* τους περιορισμούς που συνδέουν την Vᵢ με *μελλοντικές* (μη ανατεθειμένες) μεταβλητές V<0xE2><0x82><0x9A>. Αφαίρεσε τιμές από τα πεδία των V<0xE2><0x82><0x9A> που γίνονται ασυνεπείς λόγω της Vᵢ=x. Δεν ελέγχει συνέπεια *μεταξύ* των μελλοντικών μεταβλητών.
